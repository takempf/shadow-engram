...
export async function generateUniqueSlug(name: string, excludeId: number | null = null): Promise<string> {
  ...
  while (true) {
    ...
    let query = db.select({ id: Contact.id }).from(Contact).where(eq(Contact.slug, uniqueSlug));
    if (excludeId !== null) {
