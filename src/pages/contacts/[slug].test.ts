...
import {
    db, Contact, Names, EmailAddresses, PhoneNumbers, Addresses, Jobs,
    GithubProfiles, GameAccounts, StravaAccounts, Notes, eq, and
} from 'astro:db';
...
describe('Contact Page ([slug].astro) - getStaticPaths', () => {
  it('should generate correct paths with slugs from mock Contacts', async () => {
    ...
    const contactsFromDbMock = await db.select({ slug: Contact.slug }).from(Contact).all();

    const paths = contactsFromDbMock.map(c => {
      if (!c.slug) {
        console.warn(`Mocked contact is missing a slug. This shouldn't happen in test data.`);
        return null;
      }
      return {
        params: { slug: c.slug },
    ...
    for (const pathItem of paths) {
      const correspondingMockContact = mockContacts.find(mc => mc.slug === pathItem.params.slug);
...
describe('Contact Page ([slug].astro) - Page Data Logic', () => {
  ...
  beforeEach(async () => {
    ...
    const contact = await db.select().from(Contact).where(eq(Contact.slug, testContactSlug)).get();
    if (!contact) {
      contactDataForPage = { contact: null, error: "Contact not found", slug: testContactSlug };
      return;
    }
    ...
    const notes = await db.select().from(Notes).where(eq(Notes.contactId, contactId)).orderBy(Notes.createdAt).all();
    ...
    contactDataForPage = {
      contact, names, emailAddresses, phoneNumbers, addresses, jobs,
      githubProfiles, gameAccounts, stravaAccounts, notes, displayName,
  ...
  it('should fetch the main contact details correctly', () => {
    ...
    const expectedContact = mockContacts.find(c => c.slug === testContactSlug);
    expect(contactDataForPage.contact.id).toBe(expectedContact.id);
  });

  it('should determine the correct displayName', () => {
    const expectedPrimaryName = allMockNames.find(n => n.contactId === contactDataForPage.contact.id && n.name === 'primary');
    if (expectedPrimaryName) {
        ...
        const firstAnyName = allMockNames.find(n => n.contactId === contactDataForPage.contact.id);
        expect(contactDataForPage.displayName).toBe(firstAnyName ? firstAnyName.value : testContactSlug);
    }
  });

  it('should fetch associated names', () => {
    const expectedNames = allMockNames.filter(n => n.contactId === contactDataForPage.contact.id);
  ...
  it('should fetch associated email addresses', () => {
    const expectedEmails = allMockEmails.filter(e => e.contactId === contactDataForPage.contact.id);
    expect(contactDataForPage.emailAddresses.length).toBe(expectedEmails.length);
    contactDataForPage.emailAddresses.forEach(email => {
      expect(email.contactId).toBe(contactDataForPage.contact.id);
      expect(expectedEmails.some(ee => ee.id === email.id && ee.value === email.value)).toBe(true);
    });
  });

  it('should fetch associated phone numbers', () => {
    const expectedPhones = allMockPhones.filter(p => p.contactId === contactDataForPage.contact.id);
  ...
  it('should fetch addresses and the primary address value should be valid JSON', () => {
    const expectedAddresses = allMockAddresses.filter(a => a.contactId === contactDataForPage.contact.id);
    expect(contactDataForPage.addresses.length).toBe(expectedAddresses.length);
    const primaryAddress = contactDataForPage.addresses.find(a => a.name === 'primary');
    if (primaryAddress) { // Only proceed if primary address exists, which it does in mock
  ...
  it('should fetch jobs', () => {
    const expectedJobs = allMockJobs.filter(j => j.contactId === contactDataForPage.contact.id);
    expect(contactDataForPage.jobs.length).toBe(expectedJobs.length);
  });

  it('should fetch GitHub profiles', () => {
    const expectedGithubs = allMockGithubs.filter(g => g.contactId === contactDataForPage.contact.id);
    expect(contactDataForPage.githubProfiles.length).toBe(expectedGithubs.length);
  });

  it('should fetch game accounts', () => {
    const expectedGames = allMockGames.filter(g => g.contactId === contactDataForPage.contact.id);
    expect(contactDataForPage.gameAccounts.length).toBe(expectedGames.length);
  });

  it('should fetch Strava accounts', () => {
    const expectedStravas = allMockStravas.filter(s => s.contactId === contactDataForPage.contact.id);
    expect(contactDataForPage.stravaAccounts.length).toBe(expectedStravas.length);
  });
